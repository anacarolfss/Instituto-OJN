...
function VerificarDados(){
    ...
    let res_qtd_animas= qtd_animais.value;
    
    if(res_qtd_animas== 'Nenhum'){
         pontuacao+=2;
    }else if(res_qtd_animas=='1' || res_qtd_animas=='2' || res_qtd_animas== '3'){
        pontuacao+=2;
    }else if (res_qtd_animas== '4' || res_qtd_animas =='5'){
         pontuacao+=1;
    }else if(res_qtd_animas== 'mais'){
         pontuacao+=0;
    }
    ...
    // SE true nao pode adotar, quer dizer que as eliminatórias foram selecionadas
    if (eliminatoria){
        mensagem = ` Não pode adotar no momento`;
    }else if(pontuacao>=110){
        mensagem = ` Apto para adoção`;
    }else if(pontuacao >= 95){
        mensagem = "Precisa se preparar melhor";
    }else{
        mensagem = " Não recomendado no momento";
    }
    ...
    if (res_investimento_dog< 200){
        if(pontuacao>100 && eliminatoria==false ){
            mensagem += `<br> Percebemos que seu investimento mensal é mais limitado no momento, mas seu perfil demonstra responsabilidade e cuidado. Por isso, o Instituto OJN pode te apoiar com consultas gratuitas e vacinação.`
        }
